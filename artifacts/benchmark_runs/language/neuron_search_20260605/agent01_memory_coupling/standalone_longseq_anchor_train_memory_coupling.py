...
class CausalMemorySelectedThresholdBasisLowRankConvMemoryBlock(nn.Module):
    def __init__(
        self,
        *,
        dim: int,
        ...
        self.memory_basis_norm = nn.LayerNorm(memory_rank)
        self.basis_router = nn.Linear(memory_rank, basis_count)
        self.threshold_bases = nn.Parameter(torch.empty(basis_count, hidden_dim))
        self.gain_bases = nn.Parameter(torch.empty(basis_count, hidden_dim))
        nn.init.normal_(self.threshold_bases, mean=0.0, std=0.02)
        nn.init.normal_(self.gain_bases, mean=0.0, std=0.01)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        basis_weights = torch.softmax(self.basis_router(self.memory_basis_norm(memory_output)), dim=-1)
        threshold = 0.25 * torch.tanh(torch.matmul(basis_weights, self.threshold_bases))
        gain = 1.0 + 0.5 * torch.tanh(torch.matmul(basis_weights, self.gain_bases))
        preactivation = self.ffn_in(self.ffn_norm(x))
        hidden = F.relu(preactivation - threshold).square() * gain
        return x + self.dropout(self.ffn_out(hidden))


class CausalMemoryTiledThresholdLowRankConvMemoryBlock(nn.Module):
    def __init__(
        self,
        *,
        dim: int,
        ...
        self.left_paddings = [(kernel - 1) * dilation for kernel in kernels]
        self.memory_left_padding = max(1, memory_kernel_size) - 1
        self.hidden_dim = expansion * dim
        self.hidden_repeat = math.ceil(self.hidden_dim / memory_rank)
        self.conv_norm = nn.LayerNorm(dim)
        self.depthwise_layers = nn.ModuleList(
            [nn.Conv1d(dim, dim, kernel_size=kernel, dilation=dilation, groups=dim) for kernel in kernels]
        )
        ...
        self.memory_up = nn.Linear(memory_rank, dim, bias=False)
        self.ffn_norm = nn.LayerNorm(dim)
        self.ffn_in = nn.Linear(dim, self.hidden_dim)
        self.threshold_scale = nn.Parameter(torch.ones(memory_rank))
        self.gain_scale = nn.Parameter(torch.zeros(memory_rank))
        self.ffn_out = nn.Linear(self.hidden_dim, dim)
        self.dropout = nn.Dropout(dropout)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        memory_basis = torch.tanh(memory_output)
        threshold_rank = 0.20 * memory_basis * self.threshold_scale.view(1, 1, -1)
        gain_rank = 1.0 + 0.20 * memory_basis * self.gain_scale.view(1, 1, -1)
        threshold = threshold_rank.repeat_interleave(self.hidden_repeat, dim=-1)[..., : self.hidden_dim]
        gain = gain_rank.repeat_interleave(self.hidden_repeat, dim=-1)[..., : self.hidden_dim]
        preactivation = self.ffn_in(self.ffn_norm(x))
        hidden = F.relu(preactivation - threshold).square() * gain
        return x + self.dropout(self.ffn_out(hidden))
...
def make_mixer_block(config: TrainConfig, layer_index: int) -> nn.Module:
    ...
    if config.block_type == "memory_selected_threshold_basis_lowrank_conv_memory":
        return CausalMemorySelectedThresholdBasisLowRankConvMemoryBlock(
            **kwargs,
            memory_rank=config.memory_rank,
            memory_kernel_size=config.landmark_stride,
        )
    if config.block_type == "memory_tiled_threshold_lowrank_conv_memory":
        return CausalMemoryTiledThresholdLowRankConvMemoryBlock(
            **kwargs,
            memory_rank=config.memory_rank,
            memory_kernel_size=config.landmark_stride,
        )
