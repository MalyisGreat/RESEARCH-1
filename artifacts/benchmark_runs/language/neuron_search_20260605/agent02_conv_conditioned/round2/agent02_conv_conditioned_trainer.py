...
class CausalConvContextConditionedNeuronBlock(CausalMultiScaleLowRankConvMemoryBlock):
    def __init__(
        self,
        *,
        dim: int,
        ...
        self.context_gate = nn.Linear(2 * dim, expansion * dim)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        branch_stack = torch.stack(outputs, dim=0)
        conv_output = branch_stack.mean(dim=0)
        branch_disagreement = branch_stack.var(dim=0, unbiased=False)
        branch_peak = branch_stack.amax(dim=0) - branch_stack.amin(dim=0)
        x = x + self.dropout(self.mix(F.relu(conv_output).square()))
        ...
        conv_context = torch.cat((branch_disagreement, branch_peak), dim=-1)
        gate = torch.sigmoid(self.context_gate(conv_context))
        hidden = F.relu(preact).square() * (0.5 + gate)
        return x + self.dropout(self.ffn_out(hidden))
...
def make_mixer_block(config: TrainConfig, layer_index: int) -> nn.Module:
    ...
    if config.block_type == "conv_context_conditioned_neuron":
        return CausalConvContextConditionedNeuronBlock(
            **kwargs,
            memory_rank=config.memory_rank,
            memory_kernel_size=config.landmark_stride,
        )
...
def parse_args() -> TrainConfig:
    ...
    parser.add_argument(
        ...
        choices=(
            ...
            "conv_context_conditioned_neuron",
            "token_gated_lowrank_conv_memory",
            "dilated_multi_scale",
        ),
        default="relu_square",
    )
    parser.add_argument("--sequence-length", type=int, default=255)
