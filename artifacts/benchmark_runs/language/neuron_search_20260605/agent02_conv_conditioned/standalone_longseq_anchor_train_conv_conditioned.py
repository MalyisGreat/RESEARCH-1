...
class CausalBranchDisagreementConditionedLowRankConvMemoryBlock(nn.Module):
    def __init__(
        self,
        *,
        dim: int,
        ...
        self.context_down = nn.Linear(dim, memory_rank, bias=False)
        ...
        self.context_up = nn.Linear(memory_rank, 2 * hidden_dim)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        branch_outputs = torch.stack(outputs, dim=2)
        conv_output = branch_outputs.mean(dim=2)
        disagreement = (branch_outputs - conv_output.unsqueeze(2)).square().mean(dim=2)
        x = x + self.dropout(self.mix(F.relu(conv_output).square()))
        ...
        context = self.context_down(self.context_norm(disagreement)).transpose(1, 2)
        context = self.context_depthwise(F.pad(context, (self.context_left_padding, 0))).transpose(1, 2)
        shift, gain = self.context_up(F.silu(context)).chunk(2, dim=-1)
        hidden_pre = self.ffn_in(self.ffn_norm(x))
        hidden = F.relu(hidden_pre + 0.10 * torch.tanh(shift)).square()
        hidden = hidden * (1.0 + 0.25 * torch.tanh(gain))
        return x + self.dropout(self.ffn_out(hidden))


class CausalBranchDisagreementGainOnlyLowRankConvMemoryBlock(CausalBranchDisagreementConditionedLowRankConvMemoryBlock):
    def __init__(
        self,
        *,
        dim: int,
        ...
        self.context_up = nn.Linear(memory_rank, expansion * dim)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        context = self.context_down(self.context_norm(disagreement)).transpose(1, 2)
        context = self.context_depthwise(F.pad(context, (self.context_left_padding, 0))).transpose(1, 2)
        gain = self.context_up(F.silu(context))
        hidden = F.relu(self.ffn_in(self.ffn_norm(x))).square()
        hidden = hidden * (1.0 + 0.25 * torch.tanh(gain))
        return x + self.dropout(self.ffn_out(hidden))
...
def make_mixer_block(config: TrainConfig, layer_index: int) -> nn.Module:
    ...
    if config.block_type == "branch_disagreement_conditioned_lowrank_conv_memory":
        return CausalBranchDisagreementConditionedLowRankConvMemoryBlock(
            **kwargs,
            memory_rank=config.memory_rank,
            memory_kernel_size=config.landmark_stride,
        )
    if config.block_type == "branch_disagreement_gain_only_lowrank_conv_memory":
        return CausalBranchDisagreementGainOnlyLowRankConvMemoryBlock(
            **kwargs,
            memory_rank=config.memory_rank,
            memory_kernel_size=config.landmark_stride,
        )
...
def parse_args() -> TrainConfig:
    ...
    parser.add_argument(
        ...
        choices=(
            ...
            "branch_disagreement_conditioned_lowrank_conv_memory",
            "branch_disagreement_gain_only_lowrank_conv_memory",
