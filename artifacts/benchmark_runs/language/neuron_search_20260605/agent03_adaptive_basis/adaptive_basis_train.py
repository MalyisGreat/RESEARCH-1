...
class AdaptiveLocalBasisNeuron(nn.Module):
    """Token/channel-local mixture over fixed nonlinear basis responses."""

    def __init__(self, channels: int, *, base_bias: float = 2.0) -> None:
        super().__init__()
        basis_count = 4
        slopes = torch.linspace(-0.20, 0.20, basis_count).view(1, basis_count).repeat(channels, 1)
        bias = torch.zeros(channels, basis_count)
        bias[:, 0] = base_bias
        self.basis_slope = nn.Parameter(slopes)
    ...
    def forward(self, z: torch.Tensor) -> torch.Tensor:
        ...
        logits = z.unsqueeze(-1) * self.basis_slope + self.basis_bias
        weights = torch.softmax(logits.float(), dim=-1).to(dtype=z.dtype)
        return (weights * basis).sum(dim=-1)


class CausalMultiScaleLowRankConvMemoryAdaptiveBasisBlock(nn.Module):
    def __init__(
        self,
        *,
        dim: int,
        ...
        self.conv_basis = AdaptiveLocalBasisNeuron(dim)
        self.mix = nn.Linear(dim, dim, bias=False)
        self.memory_norm = nn.LayerNorm(dim)
        self.memory_down = nn.Linear(dim, memory_rank, bias=False)
        self.memory_depthwise = nn.Conv1d(
        ...
        self.memory_basis = AdaptiveLocalBasisNeuron(memory_rank)
        self.memory_up = nn.Linear(memory_rank, dim, bias=False)
        self.ffn_norm = nn.LayerNorm(dim)
        hidden_dim = expansion * dim
        self.ffn_in = nn.Linear(dim, hidden_dim)
        self.ffn_basis = AdaptiveLocalBasisNeuron(hidden_dim)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        conv_output = torch.stack(outputs, dim=0).mean(dim=0)
        x = x + self.dropout(self.mix(self.conv_basis(conv_output)))

        memory_input = self.memory_down(self.memory_norm(x)).transpose(1, 2)
        memory_output = self.memory_depthwise(F.pad(memory_input, (self.memory_left_padding, 0))).transpose(1, 2)
        x = x + self.dropout(self.memory_up(self.memory_basis(memory_output)))

        hidden = self.ffn_basis(self.ffn_in(self.ffn_norm(x)))
        return x + self.dropout(self.ffn_out(hidden))


class CausalMultiScaleLowRankConvMemoryFFNAdaptiveBasisBlock(CausalMultiScaleLowRankConvMemoryBlock):
    def __init__(
        self,
        *,
        dim: int,
        ...
        self.ffn_basis = AdaptiveLocalBasisNeuron(expansion * dim)
...
def make_mixer_block(config: TrainConfig, layer_index: int) -> nn.Module:
    ...
    if config.block_type == "multi_scale_lowrank_conv_memory_adaptive_basis":
        return CausalMultiScaleLowRankConvMemoryAdaptiveBasisBlock(
            **kwargs,
            memory_rank=config.memory_rank,
            memory_kernel_size=config.landmark_stride,
        )
    if config.block_type == "multi_scale_lowrank_conv_memory_ffn_adaptive_basis":
        return CausalMultiScaleLowRankConvMemoryFFNAdaptiveBasisBlock(
            **kwargs,
            memory_rank=config.memory_rank,
            memory_kernel_size=config.landmark_stride,
        )
...
def parse_args() -> TrainConfig:
    ...
    parser.add_argument(
        ...
        choices=(
            ...
            "multi_scale_lowrank_conv_memory_adaptive_basis",
            "multi_scale_lowrank_conv_memory_ffn_adaptive_basis",
