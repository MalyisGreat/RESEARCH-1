...
class CausalRankCompetitionLowRankConvMemoryBlock(nn.Module):
    def __init__(
        self,
        *,
        dim: int,
        ...
        self.left_paddings = [(kernel - 1) * dilation for kernel in kernels]
        self.memory_left_padding = max(1, memory_kernel_size) - 1
        self.competition_group_size = 4
        self.competition_temperature = 0.75
        self.conv_norm = nn.LayerNorm(dim)
        self.depthwise_layers = nn.ModuleList(
            [nn.Conv1d(dim, dim, kernel_size=kernel, dilation=dilation, groups=dim) for kernel in kernels]
        )
        ...
        self.ffn_in = nn.Linear(dim, expansion * dim)
        self.ffn_out = nn.Linear(expansion * dim, dim)
        self.rank_competition_logit = nn.Parameter(torch.tensor(-1.0))
        self.ffn_competition_logit = nn.Parameter(torch.tensor(-1.2))
        self.dropout = nn.Dropout(dropout)

    @staticmethod
    def local_rank_compete(
        raw: torch.Tensor,
        ...
        *,
        ...
        if channels <= 1:
            return activated
        group_size = min(max(2, group_size), channels)
        padded = (group_size - (channels % group_size)) % group_size
        prefix_shape = raw.shape[:-1]
        if padded:
            raw = F.pad(raw, (0, padded))
            activated = F.pad(activated, (0, padded))
        grouped_raw = raw.reshape(*prefix_shape, -1, group_size)
        grouped_active = activated.reshape(*prefix_shape, -1, group_size)
        centered = grouped_raw - grouped_raw.mean(dim=-1, keepdim=True)
        rms = torch.sqrt(centered.square().mean(dim=-1, keepdim=True) + eps)
        weights = group_size * torch.softmax(centered / (rms * temperature), dim=-1)
        multiplier = 1.0 + torch.sigmoid(strength) * (weights - 1.0)
        competed = (grouped_active * multiplier).reshape(*prefix_shape, channels + padded)
        if padded:
            competed = competed[..., :channels]
        return competed
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        memory_input = self.memory_down(self.memory_norm(x)).transpose(1, 2)
        memory_raw = self.memory_depthwise(F.pad(memory_input, (self.memory_left_padding, 0))).transpose(1, 2)
        memory_output = self.local_rank_compete(
            memory_raw,
            F.silu(memory_raw),
            group_size=self.competition_group_size,
            strength=self.rank_competition_logit,
            temperature=self.competition_temperature,
        )
        x = x + self.dropout(self.memory_up(memory_output))

        ffn_raw = self.ffn_in(self.ffn_norm(x))
        hidden = self.local_rank_compete(
            ffn_raw,
            F.relu(ffn_raw).square(),
            group_size=self.competition_group_size,
            strength=self.ffn_competition_logit,
            temperature=self.competition_temperature,
        )
        return x + self.dropout(self.ffn_out(hidden))


class CausalEnergyPreservingRankCompetitionLowRankConvMemoryBlock(CausalRankCompetitionLowRankConvMemoryBlock):
    @staticmethod
    def local_rank_compete(
        raw: torch.Tensor,
        activated: torch.Tensor,
        *,
        ...
        weights = group_size * torch.softmax(centered / (rms * temperature), dim=-1)
        multiplier = 1.0 + torch.sigmoid(strength) * (weights - 1.0)
        competed = grouped_active * multiplier
        source_energy = grouped_active.abs().mean(dim=-1, keepdim=True)
        competed_energy = competed.abs().mean(dim=-1, keepdim=True)
        energy_scale = (source_energy / (competed_energy + eps)).clamp(0.25, 4.0)
        competed = (competed * energy_scale).reshape(*prefix_shape, channels + padded)
        if padded:
            competed = competed[..., :channels]
        return competed
...
def make_mixer_block(config: TrainConfig, layer_index: int) -> nn.Module:
    ...
    if config.block_type == "rank_competition_lowrank_conv_memory":
        return CausalRankCompetitionLowRankConvMemoryBlock(
            **kwargs,
            memory_rank=config.memory_rank,
            memory_kernel_size=config.landmark_stride,
        )
    if config.block_type == "energy_preserving_rank_competition_lowrank_conv_memory":
        return CausalEnergyPreservingRankCompetitionLowRankConvMemoryBlock(
            **kwargs,
            memory_rank=config.memory_rank,
            memory_kernel_size=config.landmark_stride,
        )
