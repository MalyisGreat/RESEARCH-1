...
class TrainConfig:
    ...
    recall_initial_scale: float = 0.025
...
class CausalConvFactorizedLM(nn.Module):
    def __init__(self, config: TrainConfig) -> None:
        ...
        self.recall_mode = config.recall_mode
        self.recall_windows = (32, 128, 512)
        self.recall_include_prefix = self.recall_mode in {
            "token_recall_multiscale",
            "token_recall_gated_multiscale",
            "factor_recall_multiscale",
            "factor_recall_gated_multiscale",
        }
        self.recall_feature_count = len(self.recall_windows) + (1 if self.recall_include_prefix else 0)
        ...
        if self.recall_mode not in {
            "none",
            "token_recall_multiscale",
            "token_recall_gated_multiscale",
            "token_recall_recent",
            "token_recall_gated_recent",
            "factor_recall_multiscale",
            "factor_recall_gated_multiscale",
        }:
            raise ValueError(f"unknown recall_mode {self.recall_mode!r}")
        if self.recall_mode != "none":
            init = max(float(config.recall_initial_scale), 1e-6)
            inverse_softplus = math.log(math.expm1(init))
            self.recall_log_scales = nn.Parameter(torch.full((self.recall_feature_count,), inverse_softplus))
            if self.recall_mode in {
                "token_recall_gated_multiscale",
                "token_recall_gated_recent",
                "factor_recall_gated_multiscale",
            }:
                self.recall_gate = nn.Linear(config.conv_rank, self.recall_feature_count)
                nn.init.zeros_(self.recall_gate.weight)
                nn.init.zeros_(self.recall_gate.bias)
            else:
                self.recall_gate = None
        else:
            self.recall_log_scales = None
            self.recall_gate = None
    ...
    def token_recall_bias(
        ...
        *,
        ...
        if self.recall_mode == "none":
            return None
        if not self.recall_mode.startswith("token_recall"):
            return None
        if self.recall_log_scales is None:
            return None
        candidate_ids = candidate_ids.to(device=input_ids.device, dtype=input_ids.dtype)
        positions = positions.to(device=input_ids.device, dtype=torch.long)
        dtype = hidden.dtype
        if (
            candidate_ids.numel() == self.vocab_size
            and candidate_ids.numel() > 0
            and int(candidate_ids[0].item()) == 0
            and int(candidate_ids[-1].item()) == self.vocab_size - 1
        ):
            return self.full_vocab_token_recall_bias(input_ids=input_ids, hidden=hidden, positions=positions)
        hits = input_ids.unsqueeze(-1).eq(candidate_ids.view(1, 1, -1)).to(dtype)
        cumulative = hits.cumsum(dim=1)
        recall_features: list[torch.Tensor] = []
        for window in self.recall_windows:
            window_counts = cumulative.clone()
            if window < cumulative.size(1):
                window_counts[:, window:, :] = window_counts[:, window:, :] - cumulative[:, :-window, :]
            recall_features.append(window_counts.index_select(1, positions).clamp(max=1.0))
        if self.recall_include_prefix:
            recall_features.append(cumulative.index_select(1, positions).clamp(max=1.0))
        feature_stack = torch.stack(recall_features, dim=-1)
        scales = F.softplus(self.recall_log_scales).to(dtype=dtype, device=hidden.device).view(1, 1, 1, -1)
        if self.recall_gate is not None:
            gate = torch.sigmoid(self.recall_gate(hidden)).unsqueeze(2)
            return (feature_stack * scales * gate).sum(dim=-1)
        return (feature_stack * scales).sum(dim=-1)

    def full_vocab_token_recall_bias(
        self,
        *,
        input_ids: torch.Tensor,
        hidden: torch.Tensor,
        ...
        batch_size, position_count = hidden.size(0), hidden.size(1)
        bias = hidden.new_zeros((batch_size, position_count, self.vocab_size))
        scales = F.softplus(self.recall_log_scales).to(dtype=dtype, device=device).view(-1)
        if self.recall_gate is not None:
            gates = torch.sigmoid(self.recall_gate(hidden))
        else:
            gates = hidden.new_ones((batch_size, position_count, self.recall_feature_count))

        def scatter_seen(max_offset: int, feature_index: int) -> None:
            seen = hidden.new_zeros((batch_size, position_count, self.vocab_size))
            for offset in range(max_offset):
                source_positions = positions - offset
                valid = (source_positions >= 0) & (source_positions < input_ids.size(1))
                if not bool(valid.any()):
                    continue
                safe_positions = source_positions.clamp(0, input_ids.size(1) - 1)
                token_ids = input_ids.index_select(1, safe_positions).unsqueeze(-1)
                valid_values = valid.to(dtype=dtype, device=device).view(1, position_count, 1).expand(batch_size, -1, -1)
                seen.scatter_reduce_(2, token_ids, valid_values, reduce="amax", include_self=True)
            bias.add_(seen * (scales[feature_index] * gates[:, :, feature_index]).unsqueeze(-1))

        for feature_index, window in enumerate(self.recall_windows):
            scatter_seen(window, feature_index)
        if self.recall_include_prefix:
            max_prefix = int(positions.max().item()) + 1 if positions.numel() else 0
            scatter_seen(max_prefix, len(self.recall_windows))
        return bias

    def factor_recall_hidden(
        self,
        *,
        input_ids: torch.Tensor,
        hidden: torch.Tensor,
        ...
        if not self.recall_mode.startswith("factor_recall"):
            return hidden
        if self.recall_log_scales is None:
            return hidden
        positions = positions.to(device=input_ids.device, dtype=torch.long)
        dtype = hidden.dtype
        token_values = F.embedding(input_ids, self.factor_up.weight.detach()).to(dtype=dtype, device=hidden.device)
        cumulative = token_values.cumsum(dim=1)
        total_positions = token_values.size(1)
        all_counts = torch.arange(1, total_positions + 1, device=hidden.device, dtype=dtype).view(1, -1, 1)
        recall_features: list[torch.Tensor] = []
        for window in self.recall_windows:
            window_sums = cumulative.clone()
            if window < cumulative.size(1):
                window_sums[:, window:, :] = window_sums[:, window:, :] - cumulative[:, :-window, :]
            denom = all_counts.clamp(max=float(window))
            recall_features.append((window_sums / denom).index_select(1, positions))
        if self.recall_include_prefix:
            prefix_denom = (positions.to(device=hidden.device, dtype=dtype) + 1.0).view(1, -1, 1)
            recall_features.append(cumulative.index_select(1, positions) / prefix_denom)
        feature_stack = torch.stack(recall_features, dim=-1)
        scales = F.softplus(self.recall_log_scales).to(dtype=dtype, device=hidden.device).view(1, 1, 1, -1)
        if self.recall_gate is not None:
            gate = torch.sigmoid(self.recall_gate(hidden)).unsqueeze(2)
            delta = (feature_stack * scales * gate).sum(dim=-1)
        else:
            delta = (feature_stack * scales).sum(dim=-1)
        return hidden + delta
    ...
    def candidate_logits(
        ...
        *,
        ...
        hidden = self.factor_recall_hidden(input_ids=input_ids, hidden=hidden, positions=positions)
        logits = F.linear(hidden, weight, bias)
        recall_bias = self.token_recall_bias(
            input_ids=input_ids,
            candidate_ids=candidate_ids,
            hidden=hidden,
            positions=positions,
        )
        if recall_bias is not None:
            logits = logits + recall_bias
        return logits
...
def model_cross_entropy_sum_chunked(
    ...
    *,
    ...
    if model.recall_mode == "none":
        return linear_cross_entropy_sum_chunked(
            hidden,
            targets,
            weight,
            bias,
    ...
    chunk_positions = max(1, token_chunk_size // max(int(targets.size(0)), 1))
    if token_chunk_size <= 0:
        chunk_positions = targets.size(1)
    loss_sum = None
    for start in range(0, targets.size(1), chunk_positions):
        end = min(start + chunk_positions, targets.size(1))
    ...
    if loss_sum is None or token_count <= 0:
        raise RuntimeError("empty model chunked cross entropy")
    return loss_sum
...
def anchor_loss(
    ...
    *,
    ...
    sampled_sum = model_cross_entropy_sum_chunked(
        model,
        hidden,
        input_ids,
        reduced_targets,
    ...
    anchor_positions = torch.arange(0, targets.size(1), config.token_stride, device=targets.device, dtype=torch.long)
    anchor_sum = model_cross_entropy_sum_chunked(
        model,
        anchor_hidden,
        input_ids,
        anchor_targets,
        ...
        with torch.autocast(**autocast_kwargs):
            ...
            for start in range(0, hidden.size(1), config.full_eval_token_chunk_size):
                ...
                logits = model.candidate_logits(
                    ...
                    candidate_ids=torch.arange(config.vocab_size, device=device, dtype=torch.long),
...
def parse_args() -> TrainConfig:
    ...
    parser.add_argument("--recall-initial-scale", type=float, default=0.025)
    ...
    return TrainConfig(
        ...
        recall_initial_scale=args.recall_initial_scale,
    )
