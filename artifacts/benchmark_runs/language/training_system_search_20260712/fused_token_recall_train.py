...
trainer = load_module("token_recall_train_fused_experiment", TOKEN_RECALL_TRAINER)
fusion = load_module("wave10_fusion_for_training", FUSION_PROBE)
OriginalWave10Block = trainer.CausalMultiScaleLowRankConvMemoryBlock


class FusedWave10DropIn(nn.Module):
    def __init__(
        self,
        *,
        dim: int,
        ...
        self.block = fusion.FusedWave10Block(original, dilation=dilation)

    def forward(self, x):
        return self.block(x)


trainer.CausalMultiScaleLowRankConvMemoryBlock = FusedWave10DropIn
