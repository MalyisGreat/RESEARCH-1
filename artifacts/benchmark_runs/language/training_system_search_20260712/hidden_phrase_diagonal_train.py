...
class HiddenPhraseDiagonalModel(phrase.PhraseInductionModel):
    """Fuse retrieved embeddings through a learned channel-wise mixer."""

    def __init__(self, config) -> None:
        super().__init__(config)
        rng_state = torch.random.get_rng_state()
        self.hidden_retrieval_norm = nn.LayerNorm(config.embedding_dim)
        self.hidden_retrieval_channel_gain = nn.Parameter(torch.ones(config.embedding_dim))
    ...
    def features(self, input_ids: torch.Tensor) -> torch.Tensor:
        ...
        retrieved = self.hidden_retrieval_norm(retrieved_sum / retrieved_count.clamp_min(1.0))
        retrieved = retrieved * self.hidden_retrieval_channel_gain
...
trainer.CausalConvFactorizedLM = HiddenPhraseDiagonalModel
