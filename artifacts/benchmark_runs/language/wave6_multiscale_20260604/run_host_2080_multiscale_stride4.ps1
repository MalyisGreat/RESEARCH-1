...
$runName = 'wave6_host2080_multiscale_stride4_60m'
$out = Join-Path $outRoot $runName
New-Item -ItemType Directory -Force -Path $outRoot | Out-Null
Write-Host "WAVE6_LOCAL_START run=$runName host=$env:COMPUTERNAME cache=$cache"
python -u $script `
  --cache-path $cache `
  --output-dir $out `
  --run-name $runName `
