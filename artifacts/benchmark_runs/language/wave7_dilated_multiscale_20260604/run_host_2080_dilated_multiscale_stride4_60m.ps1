...
$RunRoot = Join-Path $Root 'wave7_dilated_multiscale_20260604'
...
$Out = Join-Path $RunRoot 'wave7_host2080_dilated_multiscale_stride4_60m'
$RunName = 'wave7_host2080_dilated_multiscale_stride4_60m'

Write-Host "WAVE7_LOCAL_START run=$RunName host=$env:COMPUTERNAME cache=$Cache"
python -u $Script `
  --cache-path $Cache `
  --output-dir $Out `
  --run-name $RunName `
  ...
  --val-blocks 24 `
  --embedding-dim 640 `
  --block-type dilated_multi_scale `
  --conv-layers 2 `
  --conv-kernel-size 7 `
  --conv-rank 224 `
