...
$Out = Join-Path $RunRoot 'wave8_host2080_adaptive_multiscale_stride4_60m'
$RunName = 'wave8_host2080_adaptive_multiscale_stride4_60m'

New-Item -ItemType Directory -Force -Path $RunRoot | Out-Null
Write-Host "WAVE8_ADAPTIVE_LOCAL_START run=$RunName host=$env:COMPUTERNAME cache=$Cache"
python -u $Script `
  --cache-path $Cache `
  --output-dir $Out `
  --run-name $RunName `
  ...
  --val-blocks 24 `
  --embedding-dim 640 `
  --block-type adaptive_multi_scale `
  --conv-layers 2 `
  --conv-kernel-size 7 `
  --conv-rank 224 `
...
Write-Host "WAVE8_ADAPTIVE_LOCAL_RESULT_PATH $Out\result.json"
if (Test-Path -LiteralPath (Join-Path $Out 'result.json')) {
  Get-Content -LiteralPath (Join-Path $Out 'result.json') -Raw
}
Write-Host "WAVE8_ADAPTIVE_LOCAL_DONE run=$RunName"
